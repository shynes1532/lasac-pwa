  ...
  if (enviado) {
    return (
      <div>
        ...
        <div className="bg-green-500/20 rounded-xl p-6 text-center border border-green-500/30 mt-4">
          ...
          <button onClick={() => { setEnviado(false); setTurno(VACIO); }} className="px-4 py-2 bg-white/10 rounded-lg text-sm">Nueva Solicitud</button>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white/5 rounded-xl p-4 border border-white/10">
          ...
          <div className="grid grid-cols-2 gap-2">
            {(marca.servicios || []).map((s) => (
              <button key={s.id} onClick={() => set({ servicio: s.nombre })} className={`p-2 rounded-lg text-left text-xs ${turno.servicio === s.nombre ? 'bg-brand' : 'bg-white/5'} border border-white/10`}>
          ...
          {turno.fecha && (
            <div className="mt-3">
              ...
              <div className="grid grid-cols-4 gap-1">
                {horarios.map((h) => (<button key={h} onClick={() => set({ horario: h })} className={`p-1.5 rounded text-[10px] ${turno.horario === h ? 'bg-brand' : 'bg-white/10'}`}>{h}</button>))}
              </div>
            </div>
          )}
        </div>
        <div className="bg-white/5 rounded-xl p-4 border border-white/10">
          ...
          <div className="grid grid-cols-2 gap-2">
            {sucursales.map((suc) => (
              <button key={suc.ciudad} onClick={() => set({ sucursal: suc.ciudad })} className={`p-3 rounded-lg text-left ${turno.sucursal === suc.ciudad ? 'bg-brand' : 'bg-white/5'} border border-white/10`}>
                <div className="font-bold text-sm">{suc.ciudad}</div>
                <div className="text-[10px] text-white/60">{suc.service || suc.dir || ''}</div>
              </button>
            ))}
          </div>
        </div>
        <button onClick={enviar} disabled={!completo}
          className={`w-full py-4 rounded-xl font-bold text-lg ${completo ? 'bg-gradient-to-r from-green-600 to-green-700' : 'bg-white/20 text-white/50'}`}>
          📱 Enviar por WhatsApp
        </button>
      </div>
    </div>
